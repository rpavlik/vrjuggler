 ...
 * variable \c SNX_BASE_DIR is set as soon as this DLL is attached to the
 * process. If it is not set, then it sets it based on an assumption about the
 * structure of a Sonix installation. More specifically, an assumption is made
 * that this DLL lives in the \c lib subdirectory of the Sonix installation.
 * Therefore, the root of the Sonix installation is the parent of the
      ...
      case DLL_PROCESS_ATTACH:
         {
            ...
            size_t len;
            _dupenv_s(&env_dir, &len, "SNX_BASE_DIR");
#else
            env_dir = std::getenv("SNX_BASE_DIR");
#endif
            ...
            try
            {
               ...
               // If SNX_BASE_DIR is not set, look up the path to this DLL and
               // use it to provide a default setting for that environment
               // variable.
               if ( NULL == env_dir )
               {
                  char tmppath[1024];
                  ...
                  env_stream << "SNX_BASE_DIR=" << base_dir_str;
...
#if defined(_MSC_VER) && _MSC_VER >= 1400
               _dupenv_s(&env_dir, &len, "SNX_DATA_DIR");
#else
               env_dir = std::getenv("SNX_DATA_DIR");
#endif

               // If SNX_BASE_DIR is not set, set a default relative to
               // base_dir.
               if ( NULL == env_dir )
               {
...
#else
                  std::ostringstream env_stream;
                  env_stream << "SNX_DATA_DIR=" << data_dir_str;
                  putenv(env_stream.str().c_str());
#endif
               }
#if defined(_MSC_VER) && _MSC_VER >= 1400
            ...
            {
               std::cerr << "Automatic assignment of Sonix environment "
                         << "variables failed:\n" << ex.what() << std::endl;
 ...
 * variable \c SNX_BASE_DIR is set as soon as this shared library is loaded.
 * If it is not set, then it sets it based on an assumption about the
 * structure of a Sonix installation. More specifically, an assumption is made
 * that this shared library lives in the \c lib subdirectory of the Sonix
 * installation. Therefore, the root of the Sonix installation is the parent
 * of the directory containing this shared library.
 */
extern "C" void __attribute ((constructor)) snxLibraryInit()
{
   fs::path base_dir;
   const char* env_dir = std::getenv("SNX_BASE_DIR");

   // If SNX_BASE_DIR is not set, look up the path to this shared library and
   // use it to provide a default setting for that environment variable.
   if ( NULL == env_dir )
   {
      Dl_info info;
      info.dli_fname = 0;
      const int result =
         ...
         dladdr((void*) &snxLibraryInit, &info);
#else
         dladdr(reinterpret_cast<void*>(&snxLibraryInit), &info);
#endif
      ...
      if ( 0 != result )
      {
         try
         {
            ...
            // Start the search for the root of the Sonix installation in the
            ...
            const fs::path lib_subdir(SNX_LIBDIR);
#elif defined(VPR_OS_IRIX) && defined(_ABIN32)
            const fs::path lib_subdir("lib32");
#elif defined(VPR_OS_IRIX) && defined(_ABI64) || \
      defined(VPR_OS_Linux) && defined(__x86_64__)
            const fs::path lib_subdir("lib64");
#else
            const fs::path lib_subdir("lib");
#endif
            bool found(false);
         ...
         {
            std::cerr << "Automatic assignment of SNX_BASE_DIR failed:\n"
                      << ex.what() << std::endl;
         }
      }
   }
   else
   {
      ...
      {
         std::cerr << "Invalid path set in SNX_BASE_DIR environment "
                   << "variable:\n" << ex.what() << std::endl;
      }
   }
   ...
   if ( ! base_dir.empty() )
   {
      ...
      const fs::path data_dir = base_dir / SNX_SHARE_DIR;
