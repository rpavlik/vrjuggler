      ...
      case DLL_PROCESS_ATTACH:
         {
            ...
            try
            {
               ...
               // If VJ_BASE_DIR is not set, look up the path to this DLL and
               ...
               // If VJ_BASE_DIR is not set, set a default relative to
               // base_dir.
               if ( NULL == env_dir )
               {
...
extern "C" void __attribute ((constructor)) vrjLibraryInit()
{
   ...
   // If VJ_BASE_DIR is not set, look up the path to this shared library and
   ...
   if ( NULL == env_dir )
   {
      ...
      const int result =
         ...
         dladdr((void*) &vrjLibraryInit, &info);
#else
         dladdr(reinterpret_cast<void*>(&vrjLibraryInit), &info);
#endif
      ...
      if ( 0 != result )
      {
         try
         {
            ...
            // root of the VR Juggler installation tree.
#if defined(VRJ_LIBDIR)
            const fs::path lib_subdir(VRJ_LIBDIR);
   ...
   else
   {
      ...
      {
         std::cerr << "Invalid path set in VJ_BASE_DIR environment variable:\n"
                   << ex.what() << std::endl;
      }
   }
